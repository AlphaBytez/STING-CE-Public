...
def download_report(report_id: str):
    """Download report file - demo reports are public"""
    ...
    try:
        ...
        with get_db_session() as session:
            # For demo reports, allow access by any authenticated user
            report = session.query(Report).filter(Report.id == report_id).first()
            if not report:
                return jsonify({'error': 'Report not found'}), 404

            # Check access permissions - demo reports are public, others require ownership
            if 'Demo' not in report.title and report.user_id != user_id:
...
def preview_report(report_id: str):
    """Preview report file inline (for PDF viewing in browser)"""
    try:
        user_id = get_current_user()
        if not user_id:
            return jsonify({'error': 'Authentication required'}), 401
